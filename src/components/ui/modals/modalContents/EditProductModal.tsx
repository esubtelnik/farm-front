...
import AddPhotoInput from "../helpers/AddPhotoInput";
...
const DeliveryTimeFilter = ({setValue}: {setValue: (value: number) => void}) => {
 ...
 const handleIncrement = () => {
    const newIndex = Math.min(deliveryTimeValues.length - 1, selectedIndex + 1);
    setSelectedIndex(newIndex);
    setValue(deliveryTimeValues[newIndex]);
 };
...
const EditProductModal: FC<EditProductModalProps> = ({ product }) => {
   ...
   const validateForm = () => {
      ...
      if (!form.values.image && !product.image) {
   ...
   const handleSubmit = (e: React.FormEvent) => {
    ...
    const changedFields: Partial<Record<keyof FormState["values"], FormState["values"][keyof FormState["values"]]>>
       = {};
 
    Object.entries(form.values).forEach(([key, value]) => {
       const typedKey = key as keyof FormState["values"];
 
       // Обработка изображения отдельно (null vs строка)
       if (typedKey === "image") {
          if (value) {
       ...
       const productValue = product[typedKey as keyof IProduct];
    ...
    // Пример: producerStore.updateProduct(product.id, changedFields);
 };
   ...
   return (
      <div className="bg-white max-h-[95vh] overflow-y-auto p-5 gap-y-8 flex flex-col font-geist">
         <div className="flex gap-x-5">
            <AddPhotoInput
               isEditable={true}
               imageUrl={`${import.meta.env.VITE_API_URL}${product.image}`}
         ...
         <div className="flex gap-x-5">
            ...
            >
               ...
               <input
                  ...
                  placeholder={form.errors.expirationDate ? form.errors.expirationDate : ""}
            ...
            <div className={`border-2 flex items-center gap-x-4 rounded-xl p-2 ${
                  form.errors.price
                     ? "border-red-500"
                     : "border-main-gray"
               }`}
         ...
         <div className="flex items-end justify-between w-full">
            <div className="flex flex-col w-fit items-center gap-y-2">
               ...
               <DeliveryTimeFilter setValue={(value) => handleChange("delivery", value)} />
            </div>
            <button
